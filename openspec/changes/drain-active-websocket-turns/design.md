...
- [Application drain consumes the whole deadline] → The launcher stops awaiting Uvicorn post-drain cleanup 25 seconds after the application deadline. Kubernetes termination grace reserves two seconds for failed preStop start plus 30 seconds after the helper-anchored application deadline, including five seconds for process exit; its default also retains three seconds for helper-launch headroom.
