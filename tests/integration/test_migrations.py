...
async def test_oauth_live_policy_migration_upgrade_and_downgrade(tmp_path):
    ...
    tables = {"oauth_live_policies", "oauth_live_policy_accounts"}

    def _schema_state(sync_conn):
        inspector = sa_inspect(sync_conn)
        present = {table for table in tables if inspector.has_table(table)}
        allowed_indexes = (
            {index["name"] for index in inspector.get_indexes("oauth_live_policy_accounts")}
            if "oauth_live_policy_accounts" in present
            else set()
        )
    ...
    try:
        ...
        assert "ix_oauth_live_policy_accounts_allowed_account_id" in indexes
