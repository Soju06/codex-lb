...
- Treat any destination hash as an explicit scroll intent. React Router first exposes `/firewall` as an intermediate `PUSH` location before its compatibility route replaces that entry with `/settings?advanced=1#firewall`, so the route shell also classifies that intermediary as hash intent and does not reset it. The existing Settings target-scrolling path remains the only owner of the final Firewall position.
