...
The shared stop supervisor retains listener ownership and retries transient shutdown failures with bounded delay and a bounded attempt count. If one attempt batch is exhausted, an awaiting caller receives the terminal error; the task completion callback also reports it to the process error log for unattended expiry or callback cleanup. The listener and completed stop task remain tracked, and a later browser start or full-store reset creates a fresh bounded attempt batch without publishing a replacement prematurely. Retry belongs to the shared supervisor, not the expiry watchdog, so expiry, terminal completion, durable reconciliation, reset, and partial-start cleanup all receive the same behavior.
