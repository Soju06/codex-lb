...
async def test_create_key_normalizes_timezone_aware_expiry_to_utc_naive() -> None:
    ...
    created = await service.create_key(
        ApiKeyCreateData(
            name="expiring-key",
            allowed_models=None,
            expires_at=datetime(2026, 3, 20, 23, 59, 59, tzinfo=timezone.utc),
        )
    )

    assert created.expires_at == datetime(2026, 3, 20, 23, 59, 59)

    stored = await repo.get_by_id(created.id)
    assert stored is not None
    assert stored.expires_at == datetime(2026, 3, 20, 23, 59, 59)
...
async def test_update_key_normalizes_timezone_aware_expiry_to_utc_naive() -> None:
    ...
    updated = await service.update_key(
        created.id,
        ApiKeyUpdateData(
            expires_at=datetime(2026, 4, 1, 12, 30, 0, tzinfo=timezone.utc),
            expires_at_set=True,
        ),
    )
