...
Context containers authenticate which encrypted tool results came from this proxy for the current key and the recorded source session; history-enabled forks can replay those results after their own session ownership is enforced. HTTP and WebSocket replay classification can project those exact ciphertext parts while keeping encrypted reasoning and unknown stored state account bound. In-memory verification evidence is never accepted from client JSON and is not serialized across replicas or durable restoration.
