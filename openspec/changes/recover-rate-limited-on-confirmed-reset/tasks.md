...
- [x] 1.1 Use the account/window/time-scoped usage-history lookup to require a post-block monthly baseline matching the blocked deadline within tolerance.
...
- [x] 3.1 Make reset-confirmed candidates eligible after every real selected-window reset regardless of previous usage while retaining post-reset availability, opt-in, and jitter gates.
...
- [x] 4.2 Cover recovery from persisted transition history after restart and prove the same monthly reset tuple is deduplicated.
