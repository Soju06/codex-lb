    ...
    *,
    ...
    if isinstance(parent_id, str):
        ...
        if explicit is not None:
            if not explicit.response_create_dispatched and parent_id in control.rejected_steering_parent_ids:
    ...
    if request_state not in pending_requests or (
        continuation.explicit_request_prepared and not request_state.response_create_dispatched
    ):
        if response_id is not None:
            remember_suppressed_steering_response(control, response_id)
        return None
...
def retire_steering_history_if_full(control: _WebSocketUpstreamControl) -> None:
    if (
        len(control.rejected_steering_parent_ids) + len(control.suppressed_steering_response_ids)
        >= _MAX_STEERING_HISTORY_IDS
    ):
        # Preserve correlation while admitted work drains. Requesting reconnect
        # here would make an unsent explicit replacement wait for its own reader.
...
def anonymous_websocket_request_candidates(
    pending_requests: deque[_WebSocketRequestState], control: _WebSocketUpstreamControl
) -> deque[_WebSocketRequestState]:
    if control.suppressed_steering_anonymous_terminals <= 0:
        return pending_requests
    # A suppressed successor's ID-less terminal cannot belong to an explicit
    # replacement that has not reached the transport. Keep actual live requests
    # eligible, including an explicit create whose handoff preceded this event.
    return deque(
        state
        for state in pending_requests
        if state.response_id is not None
        or state.response_create_dispatched
        or (
            state.steering_parent_response_id is None
            and state.previous_response_id not in control.rejected_steering_parent_ids
        )
    )
...
async def submit_websocket_steering(
    ...
    *,
    ...
    async with pending_lock:
        if control.retire_after_drain:
            raise steering_error("service_unavailable", "This connection is draining; retry after pending responses.")
