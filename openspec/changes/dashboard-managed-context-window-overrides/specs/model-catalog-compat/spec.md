...
- **WHEN** `PUT /api/settings/model-context-window-overrides/gpt-5.4` sends `{"context_window": 0}`, `-1` or `1.5`
- **THEN** the response is 422 and no row is written
- **WHEN** the slug segment is empty, longer than 256 characters, or contains whitespace or a control character
- **THEN** the response is 400 with code `invalid_model_slug`
