...
## Public tool-search discovery output

Hosted discovery returns tool_search_output with loaded tool definitions before the model calls a discovered tool. Public response normalization retains these definitions in item events and completed output; the separate replay-projection rules are unaffected. A response containing discovery output followed by a function call therefore reaches clients with both items intact.
