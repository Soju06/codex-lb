...
One command, marker-selected, about a minute. Every row of the table above has one `test_drill_*` rehearsal in `tests/integration/test_subscription_overflow_canary_drills.py`, running against the production route and forwarding stack. Each asserts **that row's** outcomes, not a fixed checklist: the wire answer every time -- its exact message where the row states one, and for Disconnect that nothing reached the client at all -- and then whichever of the `request_logs` rows, the `model_source_pins` rows, the `codex_lb_subscription_overflow_total{outcome}` label, the API-key reservation, the source bulkhead and the breaker state the row actually promises. No drill asserts all six, and that is the design: a drill that runs without an API key has no reservation to check, Disconnect and Silent headers pin the counters their own rows are about (`{stage}` on the abandonment counter, `{phase}` on the timeout counter) rather than the outcome label, and only Stall is a row about the breaker. So read a row and its rehearsal together, and when you change one, change the other.

Two clauses of the Kill switches row are not covered by its own drill. The **≤ 5 s settings-cache window** needs a second replica to mean anything, so it is in the manual list below. The release half -- a source-free conversation returning to a subscription account when the source goes away -- is the Neutral release drill's subject, asserted there on a *disabled* source; no rehearsal deletes a source and then releases one, and the delete leg of the Kill switches drill only pins the refusal a reasoning-bearing conversation gets.
