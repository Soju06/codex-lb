...
Per-account stream caps (`proxy_account_stream_limit`, replica-partitioned by `cap_partitioning.py`) are enforced against in-process `RuntimeState` counters under `LoadBalancer._runtime_lock`. Initial stream leases are acquired inside one of two selection paths (`_load_balancer/sticky_selection.py`, `_load_balancer/unbound_selection.py`), while an idle HTTP bridge keeps its upstream session but releases and later directly reacquires its stream lease through `LoadBalancer.acquire_account_lease`. Before this change neither path preserved API-key identity in the balancer, so nothing tracked in-flight work per key and the direct reacquisition path could bypass any selection-only gate. Local capacity denials (`account_stream_cap`) already have complete downstream semantics: `_LOCAL_ACCOUNT_CAP_ERROR_CODES` drives the transport park-and-retry loop (30s chunks, `waiting_for_account_capacity` keepalives, bounded by the per-request budget) and `LOCAL_OVERLOAD_CODES` drives 429 + `Retry-After` + `rate_limit_error` at the edge. Each park iteration re-runs selection from scratch, so any admission predicate evaluated during selection is automatically re-evaluated with fresh state.
...
- No DB I/O while holding the runtime lock and zero new locks; enabled direct reacquisition may reuse the existing cached selection-input loader before entering the lock. No new transport mechanism.
...
### D10: Idle HTTP bridge direct reacquisition is atomic and uses normal capacity

An idle HTTP bridge reacquires the selected session owner's stream lease without running a new account-selection transaction. For keyed sessions, `StreamLeaseFairShareContext` carries the API-key identity and the routing constraints needed to reconstruct the enabled gate's candidate pool before entering `_runtime_lock`. Under that lock the balancer reclaims stale leases, verifies that the direct target is still in the candidate pool, evaluates fair share, and either denies or acquires the attributed lease as one atomic operation. A filtered-out target returns the existing local-capacity failure without changing counters; no new error family or session-rebinding behavior is introduced. The direct path uses `stream_reserve_slots = 0`, matching its pre-existing normal hard-cap reacquisition contract rather than consuming the selection-time recovery reserve. Reattach and disabled-gate paths keep threshold zero, preserve attribution, and do not load candidate accounts.
