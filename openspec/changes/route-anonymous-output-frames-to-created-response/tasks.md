...
- [x] 1.1 `_match_websocket_request_state_for_anonymous_event(event_type=...)`: for an anonymous `response.*` output frame (not `response.failed` / `response.incomplete`) return the single pending request whose `response_id` is already set, visible or draining, before the draining and unresolved-visible preferences; leave error, terminal and vendor-telemetry frames and every other branch unchanged.
...
- [x] 2.1 `tests/unit/test_http_bridge_cancel_drain.py`: matcher cases for a visible created response beside a pipelined sibling, beside an unresolved draining sibling, a draining created response beside a visible sibling, two created responses staying unmatched, and a `codex.rate_limits` telemetry frame keeping pre-created ownership; bridge-reader cases proving `response.output_item.added` and `response.output_text.delta` reach the created response's queue while an anonymous `error` still pops the waiting sibling.
