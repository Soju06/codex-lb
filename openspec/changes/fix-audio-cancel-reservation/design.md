...
3. **Preserve the original cancellation if cleanup reports a failure.** The handler records a cancellation-neutral warning with source and model context, then uses bare `raise` so cleanup diagnostics never replace the request's `CancelledError`.
...
- **Cleanup itself can fail.** → Emit cancellation-neutral diagnostics while preserving the original cancellation; stale reclamation remains the final backstop for that exceptional case.
