...
- [ ] 3.2 Run `npx --yes @fission-ai/openspec@1.3.0 validate reuse-direct-wss-system-trust --strict --no-interactive` and `npx --yes @fission-ai/openspec@1.3.0 validate --specs --strict --no-interactive`; verify then sync stable requirements/context and archive only after implementation acceptance.
