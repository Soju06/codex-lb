...
- **GIVEN** a native notes call established hard `history_session` affinity
  for `session_123` on account A
- **AND** another authenticated native history request has
  `context.session_id = session_123` and no session header
- **WHEN** the proxy selects an upstream account
- **THEN** it uses the same hard `history_session` identity
