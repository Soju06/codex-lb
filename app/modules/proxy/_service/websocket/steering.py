...
async def submit_websocket_steering(
    ...
    *,
    ...
    # Steering inherits the known same-socket settings. Validate that effective
    # configuration as a fresh body so the explicit-create reset helper does
    # not inject a configuration_update into user-only steering input.
    validate_astra_request(request.model_copy(update={"previous_response_id": None}), refreshed_key)
