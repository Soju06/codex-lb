...
When an HTTP connection is lost before the in-flight response is complete, the server MUST record the loss kind — `eof` for a clean peer close, otherwise the exception type name reported to `connection_lost` — in that request's ASGI `scope["state"]` under the key `codex_lb.http_disconnected`, so the application can classify a late write as dropped. The stamp MUST write only into the existing per-request `scope["state"]` dict the server created for that request, MUST add no reference from timers or other server state to the protocol, and MUST NOT change the stock teardown (transport close on a clean close, no second close on an error close). A connection lost after the response completed MUST leave the key absent.
