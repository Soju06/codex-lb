...
- When a stream lease would be denied by the local share, the replica MAY borrow up to its fair fraction of conservative cluster headroom (`floor((configured cap − conservative cluster in-flight) / R)`), but only while every other active ring member has fresh published counts. Conservative peer usage is the greater of each published count and the peer's largest possible reserve-adjusted static share, preventing simultaneous replicas from borrowing the same heartbeat-era headroom. Missing or stale peer data disables borrowing and preserves today's static-share behavior.
