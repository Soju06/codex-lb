...
Native Codex identity and continuity headers such as `x-codex-turn-state` MUST
retain their existing values and routing semantics unless explicitly nominated
by `Connection`. Native identity classification MUST ignore nominated identity
headers. Selected authorization and account headers MUST be regenerated after
sanitation, even when their inbound equivalents were nominated.
