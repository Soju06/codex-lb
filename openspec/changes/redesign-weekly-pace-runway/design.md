...
- **Recommendation**:
  ...
  - `add_pro_accounts`: from trailing-7d demand normalized per account against
    effective capped capacity (`Σ(provider_full_credits × positive weekly burn
    percent / 100 / effective_full_credits)`) minus current fleet capacity in Pro
    quota-weeks (`Σ effective_full_credits / 50,400`), ceil,
