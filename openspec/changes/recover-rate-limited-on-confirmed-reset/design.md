...
For the selected account, the scheduler will first test its in-memory monthly before/after samples with the existing temporal reset-confirmation predicate. If that pair does not contain the transition, a blocked Free account may query persisted monthly history recorded since `blocked_at`. That history must contain a baseline whose reset deadline matches the account marker; only adjacent pairs at or after that baseline are then evaluated, and the most recent pair that passes the temporal reset predicate becomes the transition evidence. This handles upstream monthly deadlines that slide between samples without comparing non-neighboring rows.
