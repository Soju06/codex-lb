...
class DurableBridgeRepository:
    ...
    async def resolve_retained_response_alias(
        ...
        *,
        ...
        """Resolve a scoped retained alias only when its joined session and replacement target are valid."""
        statement = (
        ...
        async with sqlite_writer_section():
            ...
            if operation is not None:
                ...
                if operation.state == "failed" and handoff_allowed:
                    ...
                    operation.rebind_claim_id = None
    ...
    async def rebind_operation_for_complete_transcript(
        ...
        *,
        ...
        async with sqlite_writer_section():
            ...
            operation.rebind_claim_id = rebind_claim_id
            operation.request_text = request_text
            operation.state = "submitted"
            operation.response_id = None
            operation.event_bytes = 0
        ...
        async with sqlite_writer_section():
            ...
            operation = await self._session.scalar(
                ...
                .where(
                    ...
                    HttpBridgeOperationRecord.rebind_claim_id == rebind_claim_id,
                    *(
    ...
    async def get_operation_by_response_id(self, *, response_id: str) -> DurableBridgeOperationSnapshot | None:
        """Return the most recently updated completed or incomplete operation for a response ID."""
        operation = await self._session.scalar(
            select(HttpBridgeOperationRecord)
            .where(
            ...
            .order_by(HttpBridgeOperationRecord.updated_at.desc(), HttpBridgeOperationRecord.operation_id.asc())
            .limit(1)
        )
        return _to_operation_snapshot(operation) if operation is not None else None
    ...
    async def update_operation(
        ...
        *,
        ...
        """Update state and transcript fields only while the caller's ownership and optional generation match."""
        async with sqlite_writer_section():
            owner_exists = await self._session.scalar(
                select(HttpBridgeSessionRecord.id)
                .where(
    ...
    async def _execute_alias_upsert(
        ...
        *,
        ...
        """Upsert a scoped alias subject to recovery-target replacement rules for the active SQL dialect."""
        dialect = self._session.get_bind().dialect.name
        now = utcnow()
        values = {
            "session_id": session_id,
...
def _to_operation_snapshot(
    ...
    *,
    ...
    """Copy operation persistence and claim metadata into a detached recovery snapshot."""
    return DurableBridgeOperationSnapshot(
        ...
        rebind_claim_id=getattr(row, "rebind_claim_id", None),
