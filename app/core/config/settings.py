...
class Settings(BaseSettings):
    ...
    # Rebuild one fresh root after a websocket closes mid-response. This is an
    # at-least-once model-output strategy and is intentionally opt-in.
    http_responses_session_bridge_unsafe_partial_replay_enabled: bool = False
    ...
    http_responses_session_bridge_complete_transcript_max_bytes: int = Field(
        default=8 * 1024 * 1024, gt=0, le=128 * 1024 * 1024
    )
    ...
    # When enabled together with complete_transcript_recovery_enabled, an explicit
