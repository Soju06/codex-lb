...
export function RoutingSettings({
	...
	const cacheAffinityTtlValid =
		Number.isInteger(parsedCacheAffinityTtl) && parsedCacheAffinityTtl > 0;
	const cacheAffinityTtlChanged =
		cacheAffinityTtlValid &&
		parsedCacheAffinityTtl !== settings.openaiCacheAffinityMaxAgeSeconds;
				...
				<div className="divide-y rounded-lg border">
					<div className="flex items-center justify-between gap-4 p-3">
						...
						<Select
							...
							onValueChange={(value) =>
								save({
									upstreamStreamTransport: value as
										| "default"
										| "auto"
										| "http"
										| "websocket",
					...
					<div className="flex items-center justify-between p-3">
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Keep related requests on the same account.
							</p>
						</div>
						<Switch
					...
					<div className="flex items-center justify-between p-3">
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Bias traffic to accounts with earlier quota reset.
							</p>
						</div>
						<Switch
							...
							disabled={busy}
							onCheckedChange={(checked) =>
								save({ preferEarlierResetAccounts: checked })
							}
						/>
					</div>
					...
					<div className="flex items-center justify-between p-3">
						<div>
							<p className="text-sm font-medium">
								Allow image generation requests
							</p>
							<p className="text-xs text-muted-foreground">
								Allow <code>image_generation</code> tool calls on incoming
								OpenAI-compatible requests. This toggle is mainly for people
								using OpenCode who want to turn image generation off.
							</p>
						</div>
						<Switch
							...
							disabled={busy}
							onCheckedChange={(checked) =>
								save({ imageGenerationEnabled: checked })
							}
						/>
					</div>
					...
					<div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Keep OpenAI-style prompt-cache mappings warm for a bounded
								number of seconds.
							</p>
						</div>
							...
							<Input
								...
								onKeyDown={(event) => {
									if (event.key === "Enter" && cacheAffinityTtlChanged) {
										void save({
											openaiCacheAffinityMaxAgeSeconds: parsedCacheAffinityTtl,
										});
									}
								}}
