...
**Choice:** Remove the subquery entirely; rely on the existing `HAVING COUNT(*) ... requested_at >= since` filter (or equivalent) to keep only conversations with at least one in-window row.

**Why not invert:** "Invert" would mean "include conversation only if it has a pre-window row" — that's the opposite of the goal. The real choice was remove vs. narrow. Removing is correct: the `since` filter on the aggregated row already enforces "at least one in-window row exists" once the gate is gone, because the gate was the only thing excluding conversations that *also* had pre-window rows.
