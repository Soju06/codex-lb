...
describe("buildDashboardView", () => {
  ...
  it("adds account burn rate from per-account window consumption", () => {
    ...
    expect(burn.meta).toBe("Primary 0.7 acc/5h · Secondary 0.8 acc/7d");
  ...
  it("can hide the account burn rate card", () => {
    ...
    expect(view.stats.map((stat) => stat.label)).not.toContain("Account burn rate (5h/7d)");
  ...
  it("counts quota-exceeded secondary windows as fully burned", () => {
    ...
    expect(burn.meta).toBe("Primary 0.0 acc/5h · Secondary 1.0 acc/7d");
  });
});
