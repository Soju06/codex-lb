...
Every bundle response, including framework parsing, validation, encoding, and size failures, MUST carry no-store/no-cache headers. After atomic commit, every non-skipped account MUST receive a bounded, proxy-aware best-effort validation/usage refresh. A refresh failure MUST NOT roll back a committed row, MUST return only fixed warnings, and MUST leave unusable credentials unroutable under existing state conventions.
