...
def _ownership_table() -> sa.TableClause:
    """Build the ownership marker table expression without reflecting or mutating the database."""
    return sa.table(
        OWNERSHIP_TABLE,
...
def ensure_ownership_table(bind) -> None:
    """Create the additive-migration ownership registry only when it is absent."""
...
def was_created(bind, revision: str, object_type: str, object_name: str) -> bool:
    """Check whether this revision owns the named object, returning false when no registry exists."""
...
def forget_created(bind, revision: str, object_type: str, object_name: str) -> None:
    """Delete only the exact revision/object ownership marker, tolerating an absent registry."""
    if not sa.inspect(bind).has_table(OWNERSHIP_TABLE):
        return
    table = _ownership_table()
    bind.execute(
...
def drop_ownership_table_if_empty(bind) -> None:
    """Remove the ownership registry only after every recorded object's marker has been released."""
    if not sa.inspect(bind).has_table(OWNERSHIP_TABLE):
        return
    table = _ownership_table()
