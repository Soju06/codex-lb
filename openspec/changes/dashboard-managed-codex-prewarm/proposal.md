...
- The single consumer, `_http_bridge_prewarm_enabled`, resolves the switch from a dashboard-settings snapshot (non-NULL column wins, NULL inherits the startup `Settings`). `_maybe_prewarm_http_bridge_session` awaits the settings-cache snapshot once per new Codex session **before** taking `prewarm_lock` and reads no settings under the lock (issues #1971/#1972 wedged keyed submits on exactly that pattern).
