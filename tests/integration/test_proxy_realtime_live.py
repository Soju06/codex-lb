...
@pytest.mark.parametrize(
    ("path", "logged_path", "expected_protocol"),
    [
        (
    ...
    ids=["frameless", "current-app", "legacy-query"],
)
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("call_id", "location", "sideband_template", "expected_upstream_url"),
    [
        (
            "rtc_full_lifecycle",
    ...
    ids=["frameless", "frameless-uuid", "current-app", "legacy-query"],
)
async def test_realtime_call_location_drives_supported_account_bound_sideband_routes(
    ...
    sideband_path = sideband_template.format(call_id=returned_call_id)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "/backend-api/codex/call_not_realtime",
        "/v1/realtime",
        "/v1/realtime?call_id=rtc_first&call_id=rtc_second",
    ],
    ids=["frameless", "current-app", "legacy-missing", "legacy-duplicate"],
)
def test_realtime_sideband_websocket_rejects_malformed_call_id_before_selection(app_instance, path: str):
    with TestClient(app_instance) as client:
        with pytest.raises(WebSocketDenialResponse) as raised:
            with client.websocket_connect(
                path,
