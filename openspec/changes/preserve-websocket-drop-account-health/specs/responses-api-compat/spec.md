...
#### Scenario: reservation-store failure defers the health penalty

- **GIVEN** a dropped websocket has pending requests with unsettled API-key
  reservations
- **AND** the bounded pre-signal reservation-release attempts fail
- **WHEN** the proxy signals `stream_incomplete` so downstream cleanup can
  finish
- **THEN** the proxy retains settlement ownership and retries reservation
  release
- **AND** it records the transient account penalty only after every affected
