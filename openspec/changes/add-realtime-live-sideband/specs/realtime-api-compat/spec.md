...
- **THEN** only the exact expired row or one bounded reserved-prefix batch is removed
...
- **WHEN** the live connector raises `InvalidProxy`
- **THEN** the sideband receives the fixed credential-safe live message
- **AND** ordinary Responses WebSocket `InvalidProxy` behavior remains unchanged
...
- **AND** request logs use `request_kind=realtime_live`, `transport=websocket`, a redacted path, and no live query or credential
...
The capability MUST use existing configuration and key registration. It MUST add no setting, migration, dependency, dashboard navigation item, README section, `.env.example` line, docs page, public model entry, or public `/v1/realtime/calls` implementation. Without a registered key the private routes are unavailable, while the untouched base proxy and dashboard MUST continue to start and operate with zero new setup.
