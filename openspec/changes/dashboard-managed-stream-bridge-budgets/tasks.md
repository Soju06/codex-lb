...
- [x] 1.3 `DashboardSettingsResponse` / `DashboardSettingsUpdateRequest` fields; `upstream-connect-within-stream-budget` rule; stale source anchors in `timeout_invariants.py` refreshed.
...
- [x] 2.2 Two rows in the Upstream timeouts card with `InheritBadge`, placeholder, connect-within-stream-budget and bridge-budget-above-600 mirrors; tests.
