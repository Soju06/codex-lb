...
from app.core.balancer import RATE_LIMIT_RESET_MAX_HORIZON_SECONDS, RATE_LIMITED_MIN_COOLDOWN_SECONDS
...
async def test_implausible_persisted_reset_does_not_pin_account_after_floor(db_setup):
    """An implausible reset must not keep an available account blocked."""
    now_epoch = int(time.time())
    limited = _make_account(
        "implausible_reset",
        status=AccountStatus.RATE_LIMITED,
        blocked_at=now_epoch - 3600,
        # Keep the persisted fixture within PostgreSQL INTEGER range while
        # exceeding the accepted reset horizon. Arbitrary-size wrong-unit
        # timestamps remain covered by the pure state-builder unit tests.
        reset_at=now_epoch + 2 * RATE_LIMIT_RESET_MAX_HORIZON_SECONDS,
    )
