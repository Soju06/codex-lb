...
- Authenticate and fail closed before routing if a Daybreak Responses or compact request falls back to HTTP.
...
- Extending proactive capability lineage through the HTTP bridge, compact, retry, or reactive-policy machinery.
...
### Reject capability-bearing HTTP before routing

The static provider header follows Codex when a WebSocket attempt falls back to HTTP. External Responses and compact HTTP routes therefore validate the supplied proxy API key even when deployment-wide authentication is disabled, then return `400 required_capability_transport_unsupported` before model-source lookup, account selection, reservation, bridge creation, or upstream dispatch. Headerless HTTP requests retain the existing authentication and routing behavior. The guard uses `400`, not `426`, because Codex treats `426` as a WebSocket-to-HTTP fallback signal.
...
The user-facing examples live under `docs/examples/codex/` and are linked from `docs/client-setup.md`. The integration tests parse those exact TOML files, apply the configured provider contract to real direct Responses WebSocket and HTTP routes, and observe the first-selection or pre-routing denial boundary. An opt-in E2E regression runs an installed Codex binary with temporary `HOME` and `CODEX_HOME`, a fake API key, and a loopback-only network sandbox. It proves sibling-profile resolution, environment-key handling, initial WebSocket header emission, retained HTTP-fallback headers, and no request when the environment key is missing.
...
- **WebSocket is unavailable or compact is requested** -> Capability-bearing HTTP fails closed with a stable error before routing; direct WebSocket availability must be restored rather than dropping the carrier.
