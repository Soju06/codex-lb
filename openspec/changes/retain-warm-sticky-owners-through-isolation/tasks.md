...
- [x] 2.5 `sticky_owner_overload_isolation_reroute` carries `mapping=retained` and `substitute=deterministic|weighted`; still no account identifiers.
...
- [x] 3.3 Unchanged and green: the pool-helper contract, soft-backoff-keeps-owner, kept-when-nothing-selectable, expired-isolation and probe-reservation-sees-`effective_states` cases; `tests/unit/test_select_with_stickiness.py` pinned-owner retention.
- [x] 3.4 Regression: `tests/unit/test_overload_backoff.py tests/unit/test_load_balancer_concurrency.py tests/unit/test_select_with_stickiness.py tests/unit/test_proxy_soft_sticky_spillover.py tests/unit/test_proxy_utils.py tests/unit/test_routing_tunables.py tests/unit/test_load_balancer_contract.py`, plus the full unit suite.
- [x] 3.5 Guards: `ruff check`, `ruff format --check`, the repository architecture/simplicity checks, and `openspec validate retain-warm-sticky-owners-through-isolation --strict`.
