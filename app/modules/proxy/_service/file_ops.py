...
class _FileOpsMixin:
    ...
    async def _resolve_file_account_for_responses(
        ...
        Looks up the durable ``file_id -> account_id`` pin table built by
        ``create_file``. Used by ``/responses`` flows so a request
        ...
        if not pinned_account_ids:
            ...
            # before durable ownership was observed. With zero shared proof,
