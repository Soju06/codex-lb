...
async def _source_responses_response(
    ...
    *,
    ...
    try:
        # Inside the route-helper latch (I13): policy validation, body shaping and
        # the budget serializer can raise (a lone surrogate in the body), and a
        # claimed slot must never outlive the request that claimed it, including
        # claims already acquired by overflow routing.
        ...
        if overflow is not None:
            # Preserve the source's client-selected storage intent.
