...
The bridge can recover only when the client has resent a structurally complete logical request and durable metadata proves the retained input prefix. The suffix must retain a completed prior assistant output boundary and then supply fresh user/content input; a prefix followed only by a new user message silently drops the previous answer. Direct calls and outputs may appear before that assistant boundary, but cannot establish completeness alone because current durable metadata has no prior-output manifest capable of detecting an omitted parallel call. This structural proof cannot authenticate the assistant text itself without storing response-output content or fingerprints, so the predicate remains deliberately conservative. The fresh request must not carry any account-scoped upstream object. Removing only `previous_response_id` is insufficient when encrypted reasoning, stored conversations, uploaded files, hosted-tool state, stale turn headers, or unmatched tool outputs remain.
...
- Do not move partial continuations, conversations, encrypted content, opaque account-scoped file/container/vector handles, hosted or MCP state, or unknown request shapes across accounts. Inline data images and HTTP(S) file/image content remain self-contained inputs.
...
3. **Require complete, conservative replay proof.** Durable metadata stores the completed request's input-item count and full fingerprint. A later list-shaped full resend is eligible only when its retained prefix matches both values and its suffix proves a completed assistant `output_text`/`refusal` boundary with nonblank content followed by nonblank fresh text or valid file/image input. Direct function/custom/apply-patch calls may precede that boundary only when every represented call is settled by its matching terminal output; in-progress calls or outputs fail closed. They cannot prove completeness alone because an omitted parallel call is invisible without a persisted output manifest. A new-user-only suffix, tool-call-only suffix, empty or partial output, unresolved parallel call, mismatched output, or misordered output fails closed. The fresh copy is evaluated without `previous_response_id`; unknown top-level fields and unknown fields or malformed values in the top-level reasoning configuration fail closed. Nonblank conversation or prompt handles, upstream item identities, retained reasoning, encrypted content, opaque account-scoped file/container/vector handles, nonportable file schemes, hosted/MCP/program-mediated/unknown calls or tool choices, role-invalid or malformed message/content shapes, and outputs without matching intrinsic calls all reject replay. Assistant parts are output-only; user/system/developer parts are input-only. Inline data images and HTTP(S) file/image content are self-contained. Only self-contained direct function/custom/apply-patch histories and shape-validated account-neutral declared tools and choices are allowed; nested web-search options use exact known shapes. Apply-patch calls accept exactly one supported wire representation: an exact structured operation, a nonblank legacy patch, or a nonblank legacy input.

4. **Create a fresh recovery lane.** After a typed pre-visible owner miss, the bridge removes `previous_response_id`, strips every downstream session/turn alias, clears hard affinity, excludes the failed account, disables remote-owner forwarding for initial recovery, and uses a random server-namespaced key under the existing `internal_unanchored_parallel` durable kind. The full request is submitted once to the replacement account. No retry occurs after downstream-visible bytes.
