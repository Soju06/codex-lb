    ...
    async def put_with_request_deadline(item: str | None) -> bool:
        ...
        try:
            ...
            if not has_deadline:
                ...
                try:
                    await scheduler.wait_for(event_queue.put(item), timeout=remaining_seconds)
...
async def _enqueue_http_bridge_deferred_event(
    ...
    *,
    ...
    await _enqueue_http_bridge_event(request_state, event_queue, event_block, scheduler=scheduler, clock=clock)
        ...
        *,
        ...
        async def _publish_matched_and_terminal_frames() -> None:
            ...
            if matched_request_state is not None and matched_event_queue is not None and not suppress_downstream_event:
                for deferred_text in matched_deferred_texts:
                    await _enqueue_http_bridge_deferred_event(
                        matched_request_state,
                        matched_event_queue,
                        deferred_text,
                        scheduler=scheduler,
            ...
            if (
                ...
                if event_type in {"response.completed", "response.failed", "response.incomplete", "error"}:
                    ...
                    await _enqueue_http_bridge_event(
                        matched_request_state, matched_event_queue, event_block, scheduler=scheduler, clock=clock
                    )
