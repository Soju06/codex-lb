...
One Alembic revision parented on the single head at authoring time (`20260911_030000_add_local_login_policy`) MUST drop `dashboard_settings.password_hash`, `dashboard_settings.totp_secret_encrypted` and `dashboard_settings.totp_last_verified_step`, and MUST leave the graph with exactly one head. It MUST NOT touch `guest_password_hash`, `guest_session_generation`, `bootstrap_token_encrypted`, `bootstrap_token_hash`, `totp_required_on_login`, `totp_required_for_admin_role` or `local_login_policy`, all of which remain live. The revision MUST use `op.batch_alter_table` so SQLite is supported, MUST guard each column with an inspector check in **both** directions so re-running either body converges, and MUST NOT import anything from `app.*`. The downgrade MUST re-add the three columns as nullable **and** re-project them from the `dashboard_users` row carrying the deterministic compat id (frozen as a literal in the revision), because after this release the bootstrap account's username is no longer a stable way to find it; when that row is absent the columns MUST be left NULL. `DashboardSettings` MUST stop mapping the three columns in the same change, and `SettingsRepository.get_or_create()` MUST stop seeding them.

No earlier revision MAY damage the account rows when the chain is re-applied over a database that has already dropped the columns — a rewound ledger, or an existing schema bootstrapped without one. Every revision that reads a legacy credential column MUST first check that it is still there and stand down otherwise. That check is not sufficient on its own for the one-shot re-projection: replaying from base re-creates the columns *empty* a few revisions before dropping them again, and re-projecting from that emptiness would clear the account row, which by then holds the only credential the install has. The drop revision MUST therefore record a durable marker (a `runtime_sentinels` row, removed again by its downgrade) stating that the legacy credential layer is retired, and the re-projection MUST stand down when it finds that marker. The marker is data rather than ledger precisely so that it survives a replay.

#### Scenario: A ledger-less re-bootstrap changes no credential

- **GIVEN** a database at the drop revision whose accounts hold passwords and TOTP secrets, and whose Alembic ledger has been lost
- **WHEN** the whole chain is re-applied over that schema
- **THEN** it reaches head and every account row still carries the credential it had
...
- **THEN** each completes without error and changes nothing
