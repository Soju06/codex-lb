...
- Treat a settled marker that stays registered during the owner's close as a
  capacity placeholder: waiters observe its owner instead of adopting its
  failure or cancellation, and the stale sweeper reclaims it once wedged.
- Classify generated turn-state from recorded alias provenance on the Responses
  surface and carry signed provenance onto the forwarded owner's key.
