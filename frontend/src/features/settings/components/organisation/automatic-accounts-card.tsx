...
/**
 * Letting the company's own sign-in service add and remove people here.
 *
...
export function AutomaticAccountsCard({
  ...
  const basePath = tokensQuery.data?.basePath ?? "";
    ...
    <section id={ORGANISATION_SCIM_ID} className="scroll-mt-16 space-y-3 rounded-xl border bg-card p-5">
      ...
      <div className="space-y-1 rounded-lg border p-3">
        ...
        <div className="flex min-w-0 items-center gap-2">
          <p className="min-w-0 flex-1 font-mono text-sm font-medium break-all select-all" data-testid="scim-base-url">
            {addressFor(basePath)}
          </p>
          <CopyButton value={addressFor(basePath)} label={t("organisation.automaticAccounts.copyAddress")} />
        </div>
