...
async def db_setup(db_setup: bool) -> bool:
    """Model-created legacy fixtures must not pre-apply the unstamped fork DDL.

    The deployed usage-group revision intentionally remains immutable and is
    not idempotent. These legacy-bootstrap tests start without its Alembic
    stamp, unlike an actually deployed grouped-key database (covered separately
    by test_beta6_merge_migration), so use the real pre-group schema here.
    """
    assert db_setup
    await _prepare_pre_group_schema()
    return True


async def _prepare_pre_group_schema() -> None:
    async with SessionLocal() as session:
        await session.execute(text("DROP INDEX ix_api_keys_usage_group"))
        await session.execute(text("ALTER TABLE api_keys DROP COLUMN usage_group"))
