...
If you cannot guarantee front-door affinity, configure the deterministic bridge instance ring. Hard continuity keys that land on a non-owner replica are proxy-forwarded to the owner replica over its advertised endpoint; the proxy fails closed (`bridge_instance_mismatch` / `bridge_forward_*` errors) when the owner endpoint or ring membership cannot be resolved, the forward signature fails authentication, or an ambiguous delta requires a classifier capability that is not advertised for the durable owner's exact process epoch. In gateway-safe mode prompt-cache-only bridge requests can tolerate a locality miss and continue locally.
...
- `bridge_owner_forward_failed` with failure detail `owner_input_shape_upgrade_required`: the selected owner was not proven to run the current input-shape classifier for the process epoch recorded in durable ownership. This is an expected rolling-upgrade safety refusal before owner I/O, not an upstream failure. Finish upgrading the owner replica and verify its live ring heartbeat; if the instance restarted, allow durable ownership and its new ring advertisement to converge instead of reusing stale metadata.
