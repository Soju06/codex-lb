...
The listener exposes host DNS to containers that can reach that bridge, so limit it to the bridge gateway rather than `0.0.0.0`. A future container recreation must pass `--dns "$gateway"` or reapply the resolver override. Operators who prefer no host resolver configuration can instead recreate once with the documented Linux `--network host` launch.
