...
def _default_home_dir() -> Path:
    ...
    if getattr(sys, "frozen", False) and sys.platform == "darwin":
        # Installed macOS packages keep user data under Application Support, but
        # keep honoring an existing ~/.codex-lb during upgrades unless the user
        # has already migrated to Application Support.
...
def _env_files() -> tuple[Path, ...]:
    env_files: list[Path] = list(ENV_FILES)
    if getattr(sys, "frozen", False):
