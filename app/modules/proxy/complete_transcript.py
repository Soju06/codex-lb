...
def _terminal_output_is_valid(response: JsonValue) -> bool:
    """Accept an object response with omitted output or an array containing only output objects."""
    if not isinstance(response, dict):
        return False
    if "output" not in response:
        return True
    output = response["output"]
    return isinstance(output, list) and all(isinstance(item, dict) for item in output)
...
def materialize_output_items_from_events(events: Iterable[str]) -> list[JsonValue] | None:
    ...
    for event_text in events:
        ...
        if event_type in {"response.output_item.added", "response.output_item.done"}:
            ...
            continue
        if event_type in {"response.incomplete", "response.failed", "error", "response.cancelled"}:
            return None
...
def _normalize_input(value: JsonValue | None) -> list[JsonValue] | None:
    """Copy list input or wrap nonempty string input as a user message; reject other shapes."""
...
def _drop_bridge_operation_metadata(payload: dict[str, JsonValue]) -> None:
    """Remove the bridge's operation ID in place while preserving unrelated client metadata."""
...
def _strip_item_id(item: JsonValue) -> JsonValue:
    """Copy an output object without its response-owned ID, leaving nonobjects unchanged."""
...
def _sanitize_replay_item(item: JsonValue) -> JsonValue:
    """Remove known provider bookkeeping and empty tool-output text without masking unknown fields."""
    if not isinstance(item, dict):
        return item
...
def _sanitize_replay_content_part(part: JsonValue) -> JsonValue:
    """Copy output text without provider annotations or logprobs that fresh input cannot accept."""
...
def _is_nonblank_string(value: JsonValue | None) -> bool:
    """Recognize string metadata with at least one nonwhitespace character."""
    return isinstance(value, str) and bool(value.strip())


def _items_prefix_matches(items: list[JsonValue], prefix: list[JsonValue]) -> bool:
    """Compare an ordered prefix using replay echo rules, including optional-status compatibility."""
...
def _strip_omitted_output_subsequence(items: list[JsonValue], subsequence: list[JsonValue]) -> list[JsonValue] | None:
    ...
    around the one canonical subsequence it removes. Multiple matching
    occurrences are ambiguous and must not choose an arbitrary alignment.
    """
    ...
    if matched_start is None:
        return None
    return items[:matched_start] + items[matched_start + len(subsequence) :]


def _is_omitted_output_item(item: JsonValue) -> bool:
    """Identify reasoning or completed hosted-tool envelopes that account-neutral replay can omit."""
    return isinstance(item, dict) and (
        item.get("type") == "reasoning"
        or (item.get("type") in _OMIT_OUTPUT_TYPES and item.get("status") == "completed")
    )
...
def _canonical_item(item: JsonValue) -> str:
    """Serialize sanitized replay content deterministically without internal turn metadata."""
...
def _canonical_sanitized_without_status(sanitized: JsonValue) -> str:
    """Serialize already-sanitized echo content without optional status or internal turn metadata."""
    if isinstance(sanitized, dict):
        sanitized = dict(sanitized)
    ...
    if isinstance(left_sanitized, dict) and isinstance(right_sanitized, dict):
        if "status" in left_sanitized and "status" in right_sanitized:
            if left_sanitized["status"] != right_sanitized["status"]:
                return False
    return _canonical_sanitized_without_status(left_sanitized) == _canonical_sanitized_without_status(right_sanitized)
...
def _sanitize_output_items(raw: str) -> list[JsonValue] | None:
    """Decode stored output for replay, returning None for malformed or unfinished hosted-tool items."""
    try:
    ...
    for item in output:
        ...
        if item_type in _OMIT_OUTPUT_TYPES:
            if _is_omitted_output_item(item):
                continue
            return None
...
def _items_are_json_values(items: list[JsonValue]) -> bool:
    """Check that every top-level input item has a JSON-compatible Python value type."""
