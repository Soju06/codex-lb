...
The overview response SHALL include a comparison block that exposes whether previous-window comparison is allowed and the previous-window totals for requests, tokens, and estimated API cost when comparison metadata is available. When comparison metadata is unavailable, the dashboard client MUST accept either an omitted comparison block or `comparison: null` and MUST treat both shapes as unavailable comparison metadata. The dashboard SHALL use available comparison metadata to render a compact percentage-change indicator on the existing `Requests`, `Tokens`, and `Est. API Cost` cards only. The dashboard MUST NOT add this indicator to `Error rate` or `Account burn projection`.
...
#### Scenario: Null comparison metadata is accepted

- **WHEN** `GET /api/dashboard/overview` returns `summary.comparison: null`
- **THEN** the dashboard parses the response successfully
- **AND** the dashboard does not render previous-window percentage-change indicators
