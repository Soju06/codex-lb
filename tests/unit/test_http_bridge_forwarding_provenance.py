...
def test_synthesized_forward_keeps_both_body_proofs() -> None:
    """Generated provenance rides on the tools-bound proof; the primary rolling-upgrade
    signature stays present so predecessor owners still read an ordinary forward."""
...
def test_synthesized_marker_without_tools_bound_proof_fails_closed() -> None:
    payload = _payload()
    context = HTTPBridgeForwardContext(
        "instance-a",
        "instance-b",
