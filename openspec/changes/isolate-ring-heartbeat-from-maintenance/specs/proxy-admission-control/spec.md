...
#### Scenario: Blocked partition refresh does not delay heartbeat renewal

- **GIVEN** a cap-partition refresh remains unfinished beyond the ring-heartbeat interval
- **WHEN** ring renewal becomes due
- **THEN** the heartbeat attempt runs without waiting for the partition refresh
- **AND** the runtime does not start a duplicate partition refresh while its owner remains unfinished
