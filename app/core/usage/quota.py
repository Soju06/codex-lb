    ...
    *,
    ...
    if secondary_used is not None:
        ...
        if status == AccountStatus.QUOTA_EXCEEDED:
            # Usage data is the ground truth: if secondary_used < 100%,
            # the quota has been freed (possibly by an early upstream reset).
            # Revert to ACTIVE regardless of the stale runtime_reset timer.
    ...
    if primary_used is not None:
        ...
        if status == AccountStatus.RATE_LIMITED:
            # Same principle: primary_used < 100% means the rate limit
            # window has been freed upstream; trust usage over the timer.
            status = AccountStatus.ACTIVE
            reset_at = None
