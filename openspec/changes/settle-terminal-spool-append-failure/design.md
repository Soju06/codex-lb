...
- [A grouped terminal fan-out can stall on its first fallback] -> Queue every sibling terminal event and end-of-stream marker before beginning their sequential persistence work, settle every sibling before finalization, continue later finalizers after one fails, and preserve cancellation as the final outcome after all pre-delivered siblings finish.
...
- [Replay aliases can differ from the upstream response identity persisted at acknowledgement] -> Carry the upstream identity as the expected CAS value separately from the client-visible terminal identity; when replay resets the active upstream ID before replacement acknowledgement, retain its replay alias as the persisted identity, and preserve it when no new client-visible identity is supplied.
