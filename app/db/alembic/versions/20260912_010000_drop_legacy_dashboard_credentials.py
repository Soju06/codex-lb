...
#: Durable "the legacy credential layer is retired" marker. The ledger is not
#: enough: a database bootstrapped without an Alembic ledger replays the whole
#: chain, which re-creates these columns empty a few revisions before dropping
#: them again -- and the one-shot re-projection in between would read that
#: emptiness as "the password was removed" and clear the account row, which by
#: then holds the only credential there is. This marker is data, so it survives
#: a replay and tells the re-projection to stand down.
