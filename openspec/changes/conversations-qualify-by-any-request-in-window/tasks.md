...
- [x] 1.2 Verify `get_conversation_details()` (~lines 361-438) needs no change — it already aggregates all logs for a conversation ID regardless of window. Add an inline comment if useful to document that the detail path is intentionally window-agnostic.
...
- [x] 2.3 Add a regression test asserting that a surfaced long-running conversation reports a `started_at` strictly earlier than `since` (true global `MIN(requested_at)`), matching the "Conversation start timestamp is the true earliest request" requirement.
