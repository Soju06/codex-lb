...
@pytest.mark.asyncio
async def test_oauth_scope_uses_verified_internal_caller_and_fresh_policy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    identity = SimpleNamespace(caller_account_id="caller-internal")

    async def resolve_identity(_authorization: str | None, _account_id: str | None) -> object:
        return identity

    async def load_policy(caller_account_id: str) -> frozenset[str]:
        assert caller_account_id == "caller-internal"
    ...
    assert scope.affinity_scope_material == "oauth:caller-internal"
...
async def test_oauth_scope_fails_closed_when_policy_has_no_active_accounts(
    ...
    async def resolve_identity(_authorization: str | None, _account_id: str | None) -> object:
        return SimpleNamespace(caller_account_id="caller-internal")

    async def load_policy(_caller_account_id: str) -> frozenset[str]:
        return frozenset()
