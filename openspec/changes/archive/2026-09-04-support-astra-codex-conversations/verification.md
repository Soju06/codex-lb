...
- GitHub CI on ae922fbb found one stale failure-injection hook in `test_failed_detached_settlement_retries_failed_release_until_persisted`: it patched the former unlocked repository read while settlement now uses the locked method, so the expected failures never ran. The hook now targets `get_usage_reservation_for_update` without changing its retry/settlement assertions. `.venv/bin/python -B -m pytest -p no:cacheprovider tests/integration/test_detached_persistence.py -q`: all seven cases passed, followed by file-scoped Ruff/format/Ty checks. The other jobs in that CI run passed; the failing integration shard had 861 passing cases and eight skips.
