...
- `first_request` / `started_at` semantics are **unchanged**: they remain `MIN(requested_at)` over **all** logs for the conversation, so a conversation surfaced in a 30-day window may still report a `started_at` well before the window. This is intentional — the field reports true conversation age, not window-relative position. No new field is introduced.
...
- `get_conversation_details()` requires **no change** — it already aggregates all logs for a conversation ID regardless of window.
