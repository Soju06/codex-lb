...
def test_staged_limit_warmup_reset_threshold_is_nullable_unbackfilled_and_reversible(tmp_path: Path):
    ...
    # The staged revision owns the nullable, unbackfilled shape. The activation
    # revision that follows it makes the same column non-null with the 0.0
    # default, so this test pins the stage on the revision itself rather than
    # on the head, which the activation migration owns.
    previous_revision = "20260913_000000_add_oidc_provider_flow"
    staged_revision = "20260909_130000_stage_limit_warmup_zero_compat"
    ...
    command.upgrade(config, staged_revision)
    engine = sa.create_engine(f"sqlite:///{db_path}")
    try:
        inspector = sa.inspect(engine)
