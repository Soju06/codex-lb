...
async def _configure_route(
    app_instance: FastAPI,
    monkeypatch: pytest.MonkeyPatch,
    upstream: ScriptedUpstream,
) -> tuple[ProxyService, ApiKeyData, list[ApiKeyUsageReservationData]]:
    ...
    reservations: list[ApiKeyUsageReservationData] = []
    ...
    return service, api_key, reservations


async def _run_route(app_instance: FastAPI, socket: ScriptedSocket) -> None:
    connected = False
...
@pytest.mark.asyncio
@pytest.mark.parametrize("parent_id", ["r1", " r1 ", "\tr1\n", "\u2003r1\u2003"])
async def test_normalized_tool_continuation_releases_placeholder_before_disconnect(
    app_instance: FastAPI,
    monkeypatch: pytest.MonkeyPatch,
    parent_id: str,
) -> None:
    ...
    released_before_terminal = False

    class Socket(ScriptedSocket):
        async def send_text(self, text: str) -> None:
            nonlocal released_before_terminal
            if saw("response.completed", "r-explicit")([json.loads(text)]):
                async with SessionLocal() as session:
                    row = await ApiKeysRepository(session).get_usage_reservation(reservations[1].reservation_id)
                    released_before_terminal = row is not None and row.status == "released"
            await super().send_text(text)
    ...
    socket = Socket(
        [
            ...
            (create(parent=parent_id, input_items=[result]), saw("response.steer.pending")),
        ]
    )
    socket.finish_when = lambda event: saw("response.completed", "r-explicit")([event])
    upstream = ScriptedUpstream(
        [
            [response("response.created", "r1")],
            [
    ...
    await _run_route(app_instance, socket)
    assert await service.drain_persistence_tasks(timeout_seconds=2)
    assert released_before_terminal
    assert len(upstream.sent) == 3
    assert upstream.sent[-1]["previous_response_id"] == "r1"
    ...
    async with SessionLocal() as session:
        ...
        assert [row.status for row in rows if row is not None] == ["finalized", "released", "finalized"]
