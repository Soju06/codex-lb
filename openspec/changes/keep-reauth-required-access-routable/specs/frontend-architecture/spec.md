...
#### Scenario: Reauthentication warning accounts count in weekly credits pace

- **WHEN** an account is `reauth_required`
...
- **WHEN** no account has complete, fresh weekly credits pace data for an `active`, `reauth_required`, `rate_limited`, or `quota_exceeded` account
