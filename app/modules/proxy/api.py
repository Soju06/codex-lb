...
async def _prepend_first_task(first_task: asyncio.Task[str], stream: AsyncIterator[str]) -> AsyncIterator[str]:
    """Yield the probed first frame and settle probe cancellation without repeated cancellation cascades."""
    try:
        ...
        if not first_task.done():
            ...
            try:
                ...
                logger.warning("Failed to settle cancelled Responses probe task", exc_info=True)
    yield first
    async for line in stream:
        yield line
...
async def _collect_responses_payload(
    ...
    *,
    ...
    """Collect SSE into one response while enforcing item identity, lifecycle, and terminal consistency."""
    ...
    async for line in stream:
        ...
        if terminal_result is not None:
            if event_type in _PUBLIC_RESPONSE_STREAM_TERMINAL_TYPES:
                contract_violation_kind = contract_violation_kind or "invalid_output_item"
            continue
        if event_type == "error":
    ...
    if contract_error_kind is None and (
        output_lifecycle_invalid
        or (added_output_indexes - done_output_indexes and not isinstance(terminal_result, OpenAIErrorEnvelopeModel))
    ):
...
def _collect_output_item_event(
    ...
    *,
    ...
    """Update item capture only for valid ordered lifecycle events before response finality."""
...
def _collected_output_items_match_terminal(
    ...
    for terminal_item, collected_item in zip(terminal_output, collected_output):
        if not isinstance(terminal_item, dict):
            return False
        # Item events may already be public messages while the terminal still
        # echoes the original text-bearing extension. Compare the same public
        # representation without weakening stable identity or content checks.
        terminal_item = _normalize_public_output_item(terminal_item)
        collected_item = _normalize_public_output_item(collected_item)
        if terminal_item is None or collected_item is None:
            return False
        terminal_identity = _output_item_identity(terminal_item)
    ...
    *,
    ...
    Args:
        ...
        enforce_openai_sdk_contract: when True (the default, used for /v1),
            ...
            which feeds the Codex CLI), native event shapes including codex.*
            are preserved until terminal completion. Both modes discard late
            response data and terminal errors; SSE comments and [DONE] remain
    ...
    async for event_block in stream:
        ...
        if payload is None:
            ...
            if terminal_seen and is_unparseable_data:
                # Source-route passthrough cannot reopen a finished response.
                # Comments and [DONE] were handled before parsing above.
                continue
        ...
        raw_event_type = classify_event_type(payload)
        if (
            enforce_openai_sdk_contract
            and terminal_seen
            and isinstance(raw_event_type, str)
            and raw_event_type in _PUBLIC_RESPONSE_STREAM_TERMINAL_TYPES
        ):
            # The first terminal may already be on the wire. Abort rather
            # than emitting a second, contradictory terminal outcome.
            raise ProxyResponseError(
                502,
                openai_error("invalid_output_item", "Responses stream produced multiple terminal events"),
        ...
        if violation_kind is not None:
            contract_violation_kind = contract_violation_kind or violation_kind
            if enforce_openai_sdk_contract and raw_event_type in {"response.completed", "response.incomplete"}:
                for formatted_payload in _public_response_failed_event_blocks(
                    contract_violation_kind,
                    include_created=not created_emitted,
                    sequence_number=next_sequence_number,
                ):
                    yield formatted_payload
...
def _normalize_public_stream_payload(
    ...
    *,
    ...
    """Normalize a stream event for its public or native contract and return any violation kind."""
    event_type = classify_event_type(payload)
...
def _normalize_public_response_mapping(
    ...
    """Validate terminal echoes and normalize supported output, reporting invalid or dropped items."""
