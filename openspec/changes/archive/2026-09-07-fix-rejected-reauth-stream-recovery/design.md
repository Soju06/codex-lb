...
- Fence the post-write local routing mark with the cache generation observed before the guarded write. A repair clear or completed snapshot refresh advances that generation, so a stale rejection cannot overwrite it. A successful rejection still queues a post-write routing bump when the mark is suppressed: a snapshot read before the write must not swallow its committed status. Keep CAS misses free of speculative marks and preserve any newer unavailable state. This avoids additional database reads and uses the existing invalidation-bus convergence bound.
