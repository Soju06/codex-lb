...
@pytest.mark.asyncio
async def test_scheduler_stop_cancels_and_awaits_pending_version_warmup(monkeypatch: pytest.MonkeyPatch) -> None:
    started = asyncio.Event()
    stopped = asyncio.Event()

    async def pending_version() -> str:
        started.set()
        try:
    ...
    monkeypatch.setattr(
        scheduler_module, "get_codex_version_cache", lambda: SimpleNamespace(get_version=pending_version)
    )
    refresh = AsyncMock()
...
@pytest.mark.asyncio
async def test_disabled_model_scheduler_does_not_start_version_warmup(monkeypatch: pytest.MonkeyPatch) -> None:
    get_version = AsyncMock()
    monkeypatch.setattr(scheduler_module, "get_codex_version_cache", lambda: SimpleNamespace(get_version=get_version))
    scheduler = scheduler_module.ModelRefreshScheduler(interval_seconds=3600, enabled=False)
    await scheduler.start()
    await scheduler.stop()
    assert scheduler._task is None
