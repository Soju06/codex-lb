...
- [x] 1.3 `_http_bridge_prewarm_enabled(settings, dashboard_settings)` resolves the dashboard column over the env alias; `_maybe_prewarm_http_bridge_session` awaits the settings-cache snapshot once before `prewarm_lock` and reads no settings under the lock.
...
- [x] 3.1 Unit: resolver states (NULL → env, NULL + env unset → default, dashboard value including `false`), prewarm path honours dashboard over env in both directions, exactly one snapshot read before the lock with zero session-factory calls (spy).
