...
- Keep stored access tokens usable until upstream rejects them.
...
### Keep failure exclusion request-scoped

A permanent forced-refresh failure releases the current lease and excludes that account from the request's remaining movable retries. It does not create a process-wide unavailable mark.

### Preserve affinity

A refresh warning does not prove owner loss. Sticky, bridge, file, response, and realtime continuity therefore remain bound to the same account; hard-unavailable states retain existing cleanup behavior.

## Risks

- An expired access token may cause one rejection per independent request; request-local exclusion prevents loops and permits movable failover.
