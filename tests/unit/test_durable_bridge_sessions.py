...
@pytest.mark.asyncio
async def test_detached_row_reclaim_does_not_let_second_replica_steal_live_owner(
    async_session_factory: Callable[[], AsyncSession],
    coordinator: DurableBridgeSessionCoordinator,
) -> None:
    claimed = await coordinator.claim_live_session(
        session_key_kind="thread_header",
        session_key_value="thread-detached-race",
        api_key_id="key-1",
        instance_id="instance-a",
    ...
    first = await coordinator.lookup_request_targets(
        session_key_kind="thread_header",
        session_key_value="thread-detached-race",
        api_key_id="key-1",
        turn_state=None,
    ...
    second = await coordinator.lookup_request_targets(
        session_key_kind="thread_header",
        session_key_value="thread-detached-race",
        api_key_id="key-1",
        turn_state=None,
    ...
    assert winner.session_id == claimed.session_id
    ...
    loser = await coordinator.claim_live_session(
        ...
        api_key_id="key-1",
        instance_id="instance-c",
        owner_process_epoch="test-process-c",
        lease_ttl_seconds=120.0,
        account_id="acc-replacement",
        model="gpt-5.4",
        service_tier=None,
    ...
    assert loser.session_id == claimed.session_id
    assert loser.owner_instance_id == "instance-b"
    assert loser.owner_epoch == winner.owner_epoch
    assert loser.state == HttpBridgeSessionState.ACTIVE
    assert loser.lease_is_active(now=utcnow()) is True
