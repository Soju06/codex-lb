...
@pytest.mark.asyncio
async def test_dashboard_account_scoped_thread_identity_migration_upgrade_and_downgrade(tmp_path):
    """Upgrade adds the nullable ``dashboard_settings.account_scoped_thread_identity_enabled``
    column, downgrade drops it, and a final walk to head proves the revision sits
    on a single-head graph. The parent is read from the script so re-chaining the
    revision at merge time does not break the test."""
    from alembic import command
    from alembic.script import ScriptDirectory

    from app.db.migrate import _build_alembic_config

    db_url = f"sqlite+aiosqlite:///{tmp_path / 'dashboard-scoped-identity.sqlite'}"
    revision = "20260911_000000_dashboard_account_scoped_thread_identity"
    column = "account_scoped_thread_identity_enabled"
    config = _build_alembic_config(db_url)
    parent_revision = ScriptDirectory.from_config(config).get_revision(revision).down_revision
    assert isinstance(parent_revision, str)
