...
- **AND** the replay MUST keep the response identity the client is already reading: the replay's own `response.created` MUST be suppressed downstream and every later event MUST carry the accepted response's ID, so no client ever observes a second `response.created` or a changed response ID mid-stream
...
- **THEN** the bridge MUST wait for a bounded transient backoff and replay the unchanged request exactly once on the same account
- **AND** the replay MUST preserve the original parent `previous_response_id` and the accepted response identity already delivered downstream
- **AND** the bridge MUST NOT perform this accepted-response replay for public OpenAI SDK streams, after any reasoning, text, item, or tool output, after any frame carrying a finite integer `sequence_number` has reached the client, while another request is pending, or after the replay budget is exhausted
- **AND** when an untyped transport close is not replayed because those safety gates block replay or the replay budget is exhausted, the bridge MUST fall through to the existing pre-created retry circuit and its close classification, unchanged
