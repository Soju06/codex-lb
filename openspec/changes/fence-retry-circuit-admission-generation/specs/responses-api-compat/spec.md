...
Local request deadlines and relative lease budgets MUST use the owning
service's clock. Durable claim and diagnostic lookup waits, spawned claim
tasks, release retries, and background cleanup drain bounds MUST use the
owning service's scheduler. Injecting those collaborators MUST NOT change
generation fencing, cancellation settlement, or reconciliation limits.
...
#### Scenario: Virtual time preserves the claim reconciliation budget

- **GIVEN** the service has an injected clock and scheduler
- **AND** the durable claim does not complete before its timeout
- **WHEN** the scheduler advances to the caller's deadline after one attempt
- **THEN** the service MUST return an undecided outcome without a second claim
- **BUT WHEN** the first operation settles cancellation with enough budget
  for one more attempt
- **THEN** the service MAY run exactly one reconciliation on the same scheduler
- **AND** the reconciliation MUST stop at the original caller deadline
...
- **GIVEN** the merged automation claim-budget parent revision is applied with
  ...
  fold watermarks, captured and legacy NULL automation budgets, and retry-circuit
  generations
...
- **AND** all upstream schemas, settings, transport choices, pins, report history,
  fold watermarks, automation budgets, and retry generations MUST remain intact
  while only the nullable claim-marker columns are added or removed
