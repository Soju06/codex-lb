...
functional area; it SHALL document the bare `PORT` special case, the
`CODEX_LB_ENV_FILE` bootstrap variable, and the `CODEX_LB_WORKERS_PER_INSTANCE`
startup guard (which is not a setting), and SHALL list the removed
(`_REMOVED_SETTINGS`) env names sourced from the code. The generated page
