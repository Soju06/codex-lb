...
| `CODEX_LB_TOKEN_REFRESH_INTERVAL_DAYS` (#PR) | `extraEnv` only | None: the proactive token-refresh window is a fixed eight days (`TOKEN_REFRESH_INTERVAL_DAYS` in `app/core/auth/refresh.py`), which is the former default. An account is still refreshed on demand whenever upstream answers 401, so shortening the window was never a recovery lever; lengthening it only delayed a refresh that had to happen anyway. |
