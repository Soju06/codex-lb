...
- **Backend (new)**: `app/core/clients/rate_limit_reset_credits.py` (upstream client), `app/modules/rate_limit_reset_credits/store.py` (in-memory store + singleton), `app/core/usage/reset_credits_refresh_scheduler.py` (60s leader-gated loop), `app/modules/rate_limit_reset_credits/api.py` (dashboard GET + consume POST).
