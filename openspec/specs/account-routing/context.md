...
Accounts-page caps reserve standard subscription quota without pausing accounts or changing provider totals. For example, 80% usable on 5h and 50% usable weekly stops new admission at 20% or 50% remaining respectively, whichever comes first. Blank controls disable each cap independently; both start disabled.
...
The nullable database columns require the normal schema upgrade. No environment variables or new dependencies are needed. The Accounts and Dashboard account bars retain provider percentages and mark the reserved threshold. Aggregate 5h and weekly credit donuts and weekly pace subtract reserved capacity so their totals reflect quota the proxy may actually route.
