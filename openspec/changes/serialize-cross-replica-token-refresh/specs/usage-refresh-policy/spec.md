...
- **GIVEN** a replica completed a successful upstream token exchange and holds the newly issued single-use tokens
- **AND** the guarded persistence compare-and-set keeps missing on a sustained same-plaintext re-encryption storm until the bounded retry budget is exhausted, with no genuinely different peer rotation ever observed
- **WHEN** the replica cannot win an atomic compare-and-set window before the bounded retries are exhausted
- **THEN** it raises a transient (non-permanent) `token_persist_conflict` refresh error that is not recorded in the permanent-failure cooldown, so the caller retries the whole refresh once the contention clears
