...
- Retire only the denied `previous_response_id` on the first explicit upstream denial when the proxy injected it, instead of waiting for a counter that this failure class never increments. Record the denial durably independent of the current owner epoch so detached generations and other replicas can publish the fact. In the same transaction, clear the four anchor-bound columns only while that id is still the durable latest response and delete only that response alias, preserving a newer anchor, turn-state, and other response aliases. The matching in-memory carrier is cleared in a `finally` path even when alias unregistering fails.
- Mark the denied id before the durable await and revalidate prepared requests immediately before dispatch. The final durable recheck and the denial write share one session-row fence that is held through the WebSocket send, so a request that already captured the denied proxy-injected id either finishes sending before durable publication or fails closed without another upstream send. Bound the fenced send by the existing upstream transport timeout, roll back a recovery alias if fence entry fails before dispatch, and contain local alias-unregister failures so none of these bookkeeping paths can hold the database fence indefinitely or penalize account health.
- If the first durable clear fails, retry it when the same live session reads the surviving tombstoned id, while suppressing that stale lookup from hydration, injection, and prefix trimming. If request preparation raced ahead of resolving the process-local tombstone, rebuild the request from the client's original full resend before dispatch so cleanup failure cannot become a permanent pre-dispatch 502 loop.
- Carry tombstones across live same-key generations and replicas: publish to an already-installed successor, inherit from detached predecessors during registration, persist the denial in the existing durable alias table, strip a known-denied durable anchor before request-level payload preparation, and recheck it before alias publication and send.
