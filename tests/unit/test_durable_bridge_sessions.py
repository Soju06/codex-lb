...
async def test_coordinator_forwards_unknown_operation_lookups(
    ...
    """Coordinator lookup wrappers forward all recovery bounds and scope fields to the repository."""
...
async def test_durable_bridge_previous_response_alias_lookup_returns_replacement_anchor(
    ...
    """Looking up a retained previous-response alias returns its replacement upstream anchor."""
    claimed = await coordinator.claim_live_session(
        session_key_kind="session_header",
...
async def test_durable_bridge_lookup_prefers_retained_alias_during_same_account_handoff(
    ...
    """Same-account handoff still respects a retained alias instead of reviving its obsolete target."""
...
async def test_retained_response_alias_requires_replacement_target(
    ...
    """Retained alias registration fails closed when no replacement response target is supplied."""
