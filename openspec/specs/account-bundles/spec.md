...
Destination email lookup MUST apply the database's `lower` function to both stored and source email (`lower(accounts.email) = lower(source_email)`), preserving the indexed column expression and legacy database case-conversion semantics. Source-payload duplicate validation MUST continue to use Unicode lowercase without trimming. Repeated ordinary `auth.json` imports with identical non-ASCII email MUST reuse the existing slot.

#### Scenario: Bundle ignores a destination awaiting deletion
- **WHEN** the only matching destination has a non-null deletion-request timestamp
- **THEN** preflight reports a new record and either skip or replace commit creates a distinct visible account without changing the deletion-marked account

#### Scenario: Repeated Unicode single-account import
- **WHEN** the same `auth.json` with email `Üser@example.com` is imported twice on SQLite
- **THEN** both imports return the same account id and only one visible account exists
