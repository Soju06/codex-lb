...
def build_owner_forward_headers(
    *,
    ...
    # forward. Its wire bytes stay byte-compatible with pre-provenance owners.
    ...
    return _with_bridge_turn_state_provenance(
        forwarded,
        context=context,
        authenticated_body_signature=tools_bound_signature,
    )
...
def parse_forwarded_request(
    ...
    *,
    ...
    if tools_bound_valid or context.downstream_turn_state_synthesized:
        return _authenticated_bridge_forward_result(
            headers=headers,
            context=context,
            authenticated_body_signature=tools_bound_signature if tools_bound_valid else None,
        )
...
def _with_bridge_turn_state_provenance(
    ...
    *,
    ...
    """Bind synthesized-state privilege to the emitted tools-bound body proof."""
...
def _authenticated_bridge_forward_result(
    *,
    headers: Mapping[str, str],
    context: HTTPBridgeForwardContext,
    authenticated_body_signature: str | None,
) -> tuple[HTTPBridgeForwardedRequest | None, ProxyResponseError | None]:
    """Grant synthesized-state privilege only for the validated body proof.

    ``authenticated_body_signature`` is the tools-bound signature this receiver
    already verified for this exact body, or ``None`` when no body proof
    validated; a synthesized marker without one fails closed.
    """
    ...
    if context.downstream_turn_state_synthesized:
        ...
        provenance_valid = (
            provenance_signature is not None
            and authenticated_body_signature is not None
