...
## Observed generation timing and sample quality

Request logs preserve total request latency separately from the upstream terminal observation. The dashboard speed estimate uses non-reasoning output tokens over first non-reasoning output to terminal; visible reasoning can still establish TTFT. This does not change the account-routing throughput cohort, which retains its own total-output sampling rule. Bridge timing is recorded when the upstream event is received, before queue delivery. All timestamps share the existing owner clock and attempt/request-state anchor.

Two chunks and a 100 ms output window qualify an observed estimate; they do not measure model-internal decode speed or impose a speed ceiling. Ten tokens delivered in one five-millisecond chunk have insufficient sampling evidence. Missing reasoning counts remain unknown. Historical rows retain nullable timing evidence and a separately labelled historical estimate when the old inputs are valid; those estimates do not enter daily TPS medians. Empty medians remain null and eligible sample counts are shown. Existing report range limits and soft-deleted history retention stay intact.
