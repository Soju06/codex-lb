...
def project_responses_input_for_account_neutral_fresh_replay(
    ...
    *,
    ...
    canonical_lite_developer_index: int | None = None
    raw_canonical_lite_developer_index = _canonical_lite_developer_index(input_items, stored_count=stored_count)
    for index, item in enumerate(input_items):
        ...
        if projected_item is not None:
            ...
            if index == raw_canonical_lite_developer_index and len(projected_items) == 2:
...
def _canonical_lite_developer_index(input_items: list[JsonValue], *, stored_count: int) -> int | None:
    """Return the raw index of the canonical Responses-Lite developer message.

    The canonical shape is an ``additional_tools`` bundle at index 0 whose
    immediate successor is an inline developer message, both inside the stored
    prefix. Raw-item readers must use this rather than a projected index,
    because projection may drop items and shift positions.
    """

    if (
        stored_count >= 2
        and _is_canonical_lite_tool_bundle(input_items[0])
        and _is_inline_developer_message(input_items[1])
    ):
        return 1
    return None
...
def responses_input_suffix_has_response_owned_prefix_settling_output_ids(
    ...
    *,
    ...
    This runs on the raw client body before any projection, so the canonical
    Responses-Lite developer position is recomputed against the raw items. A
    prefix that cannot be parsed is reported as response-owned: the caller
    treats ``True`` as "not provably clean", which is the safe verdict.
    """
    ...
    prefix_state = _direct_tool_call_prefix_state(
        input_items[:stored_count],
        allow_historical_developer_interleave=True,
        canonical_lite_developer_index=_canonical_lite_developer_index(input_items, stored_count=stored_count),
    )
    if prefix_state is None:
        return True
