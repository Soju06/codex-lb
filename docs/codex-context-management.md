...
The deployed ownership revision keeps its original August 30 parent. Merge revisions join it to the upstream migrations, including the dashboard user migration chain. Startup therefore applies intervening upstream changes on existing context installations, while fresh upstream databases create the context tables. These paths and the previous context merge reach `20260910_180000_merge_context_dashboard_heads` without manual version stamping. Existing dashboard credentials are carried into the upstream user model. Preserve a consistent database backup and its encryption key before upgrading.
