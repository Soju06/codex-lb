...
The shared pricing registry resolves exact model IDs first and otherwise selects the longest matching wildcard alias. Because GPT-5.6 has no pricing entries or aliases, all three personality slugs currently resolve through `gpt-5*` to the canonical `gpt-5` price.
...
- resolve exact and suffixed GPT-5.6 personality slugs to the correct canonical price
