...
- [x] 5.7 Report accepted `model_at_capacity` terminals as `server_is_overloaded` (pre-created classifier parity) so the websocket owner-switch path handles anchored accepted turns; classifier, text-path, and integration tests over both codes.
