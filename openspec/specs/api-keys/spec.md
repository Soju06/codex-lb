...
When an API key has account-assignment scope and this policy is configured, account selection SHALL exclude an assigned account when the key's successful, attributable request-log cost for that account in the account secondary quota window reaches the configured percentage of that account's local quota budget. The local quota budget MUST be projected into request-cost units from the account's secondary plan capacity, its live upstream used percentage, and the total successful local API-key request-log cost attributed to that account in the same window. The system MUST NOT compare request-log USD directly to plan-capacity credits or create an account-wide operator cap.
