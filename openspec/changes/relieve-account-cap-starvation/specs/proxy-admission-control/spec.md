...
For stream leases only, a replica whose reserve-adjusted local share is exhausted MAY additionally borrow from observed idle peer capacity: with fresh per-account in-flight stream-lease counts published by every other active ring member, the borrow allowance for an account MUST be `floor((configured stream cap − conservative cluster in-flight) / R)`, floored at zero. Conservative cluster in-flight MUST be this replica's local in-flight count plus, for every peer, the greater of that peer's freshest published count for the account and the largest reserve-adjusted static share any replica can hold under the current partition. This peer floor MUST reserve capacity each peer could have admitted since its last heartbeat so simultaneous replicas cannot borrow the same stale headroom. Borrowing MUST be disabled — preserving the static share — whenever any other active member's published counts are missing or staler than the ring stale threshold, whenever the configured stream cap is nonpositive, whenever `proxy_account_caps_scope` is `replica`, and whenever the partition has a single member. Snapshot coverage MUST be validated against the active peer identities observed by the latest membership refresh, not by member count alone: when the covered peer set differs from the current active peers (for example a same-count rolling replacement, or a failed peer-count refresh after a newer membership view), the snapshot MUST be treated as missing for both borrowing and the static-share clamp. The stream recovery reserve MUST be applied to the peer static-share floor exactly as to the local static share for non-recovery admissions. Borrowed admissions MUST be observable via a dedicated metric. While fresh complete peer counts are available, static-share admission MUST likewise be clamped by the remaining cluster headroom (the configured stream cap minus peers' published in-flight counts, less the applicable stream recovery reserve) so capacity already borrowed by peers is not re-admitted locally; without fresh complete counts the static share applies unchanged as the fail-safe entitlement. The conservative peer floor and fair-fraction allowance MUST keep simultaneous heartbeat-era borrows at or below the configured cluster cap.
...
- **AND** non-recovery admission reserves one stream slot from each static share (usable share 3 each)
- **AND** the peer replica's fresh published in-flight count for account A is 0
- **AND** this replica holds 3 in-flight stream leases for account A
- **WHEN** a new stream lease for account A is requested on this replica
- **THEN** conservative peer usage is floored at the peer's usable static share of 3
- **AND** the borrow allowance is `floor((8 − 3 − 3) / 2) = 1`
- **AND** the lease is admitted instead of rejected with `account_stream_cap`

#### Scenario: Simultaneous replicas cannot borrow the same stale headroom

- **GIVEN** a configured account stream cap of 8 partitioned across two replicas with a reserve-adjusted usable share of 3 each
- **AND** both replicas last published 0 in-flight streams for account A
- **WHEN** both replicas reach 3 local streams and calculate a borrow before the next heartbeat
- **THEN** each replica's borrow allowance is at most 1
- **AND** the aggregate after both borrows is at most the configured cap of 8

#### Scenario: Peer borrows clamp static-share admission

- **GIVEN** a configured account stream cap of 8 partitioned across two replicas (share 4 each)
- **AND** the peer replica's fresh published in-flight count for account A is 5 (it borrowed while this replica was below its share)
- **AND** this replica holds 3 in-flight stream leases for account A
- **WHEN** a new stream lease for account A is requested on this replica
- **THEN** the lease is denied even though this replica is below its static share
- **AND** the sustained aggregate stays at the configured cap of 8

#### Scenario: A same-count peer replacement disables borrowing until counts cover the new peer

- **GIVEN** two active replicas where the peer instance is replaced by a new instance id between heartbeats (member count unchanged)
- **AND** a still-fresh counts snapshot covering only the departed peer
- **WHEN** a share-exhausted account requests a stream lease on this replica
- **THEN** no borrow allowance is granted until a snapshot covering the new peer's published counts is observed
