...
- Return only non-empty daily buckets from the API. The frontend fills the complete rolling 180-day calendar range so zero-request days remain visible.
...
The new endpoint will reuse the existing watermark-aware hourly data, but the aggregation boundary is the requested timezone's local day rather than UTC midnight. The folded SQL statements should process bounded batches of labeled local-day ranges; each statement obtains the watermark from `AccountUsageRollupState` and `LEFT JOIN`s the hourly rollup source, represents each completed local day with its independently converted UTC start and following-midnight end, represents the current local day with its independently converted UTC start and the captured current instant as its end, constrains folded rows by the hourly rollup time-bucket key range, and groups whole folded UTC-hour buckets by the bounded local-day label. Raw reads must cover exactly the requested UTC segments not represented by those whole folded buckets: the bounded post-watermark segment and any bounded pre-watermark partial edge windows where a local-day boundary falls between UTC hour boundaries. Those raw queries group by the same local-day label in SQL. Application code merges the resulting small per-day maps across folded and raw batches. This preserves per-batch watermark consistency without per-grain materialization and avoids an unbounded `request_logs` query or per-day application-side table scan.
...
- Test heatmap calendar construction from browser-local date components, levels, and tooltip content, including the exactly 180-day unlabeled layout and a fixed refresh interval.
...
- Test the repository/service aggregation over folded and raw-tail data, including bounded watermark-consistent `AccountUsageRollupState LEFT JOIN` rollup batches, exact raw-complement bounds with pre-watermark partial edge windows, local-day grouping, per-day application merge across batches, warmup exclusion, empty days, and the documented retention-pruned partial-hour undercount limitation.
