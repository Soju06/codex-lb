...
from app.modules.proxy._service.http_bridge.helpers import (
    ...
    _http_bridge_capacity_wait_future_locked,
    ...
    _observe_http_bridge_retained_inflight_marker,
        ...
        *,
        ...
        while True:
                    ...
                    if (
                        ...
                        if (
                            ...
                            if _http_bridge_inflight_creation_count(self):
                                capacity_wait_future = _http_bridge_capacity_wait_future_locked(self)
                            else:
            ...
            if capacity_wait_future is not None:
                ...
                if _http_bridge_inflight_marker_is_settled(capacity_wait_future):
                    # A settled marker is a capacity placeholder for a creator that
                    # is still finalizing; its outcome is not this request's.
                    await _observe_http_bridge_retained_inflight_marker(
                ...
                try:
                    ...
                    if exc is _http_bridge_inflight_abort_error(capacity_wait_future):
                        # Admission-side eviction of the marker this request
                        # observed: the structured local-overload 429 applies
                        # to every waiter of that marker.
                        raise
                    # The creator's own rejection (for example a 402 quota
                    # error) belongs to the creator's key, not to this one.
                    await _observe_http_bridge_retained_inflight_marker(
                        self, capacity_wait_future, wait_timeout_seconds, request_deadline
                    )
                except Exception:
            ...
            if inflight_future is not None and not owns_creation:
                ...
                if inflight_future.cancelled():
                    # The owner still holds the slot while closing; awaiting the
                    # cancelled future would spin until it pops the marker, so
                    # observe the owner. A creator error is still adopted below.
                    await _observe_http_bridge_retained_inflight_marker(
