...
#### Scenario: Default reclaim window follows the default compact budget

- **GIVEN** default configuration, where `compact_request_budget_seconds` is 900 seconds
- **WHEN** a claimed run is abandoned without an orderly release (for example the replica is killed mid-flight)
- **THEN** the slot becomes reclaimable once the claim has been held longer than 930 seconds (budget plus the 30-second grace)
- **AND** the automation's own compact request is bounded by the same 900-second budget
