...
- Add a forward-only Alembic migration `20260717_000000_optimize_dashboard_hot_path_indexes` that:
  ...
  - Drops the redundant indexes `idx_logs_requested_at` (⊂ `idx_logs_requested_at_id`), `idx_logs_api_key_time_account` (interchangeable with `idx_logs_api_key_time`; zero scans in production), and `ix_additional_usage_history_account_id` (⊂ the new labels index) — on PostgreSQL via `DROP INDEX CONCURRENTLY` so a hot `request_logs` never queues writers behind an `ACCESS EXCLUSIVE` lock during startup migration.
