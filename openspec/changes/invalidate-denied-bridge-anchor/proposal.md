...
- If the first durable clear fails, retry it when the same live session reads the surviving tombstoned id, while suppressing that stale lookup from hydration, injection, and prefix trimming so cleanup failure cannot become a permanent pre-dispatch 502 loop.
