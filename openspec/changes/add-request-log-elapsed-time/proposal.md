...
- Show `Elapsed Time` in the Request Details dialog as `${elapsed} (${latency})`, with the secondary latency value rendered in smaller dimmer text.
...
- `frontend-architecture`: dashboard request-log payloads and Request Details rendering must expose and display elapsed timing plus the existing latency timing together.
