...
## Estimated usage-share admission seam

HTTP subscription routes enforce usage share at the public request boundary after model-source routing and provisional fixed-limit reservation, but before subscription dispatch or HTTP-bridge forwarding. The public origin therefore owns the decision; a bridge owner trusts the signed forwarded request and does not evaluate the share again. Responses and Chat Completions (including Images), compact, and transcription use this route-level seam. Model-source, metadata/control, and file-control work bypass it.

Direct Responses WebSockets rebuild the key policy for every fresh client `response.create`. Source routing and continuity checks run first. A subscription turn is then admitted exactly once: when opening a new upstream socket, or immediately before registering a turn that will reuse an existing one. Transparent reattach inherits its original admission. This placement covers socket reuse without coupling policy to account selection or charging bounded failover and replay again.

A denial's `resets_at` is the earliest reset among contributing account windows: it is the next point at which the pool estimate may change, not a promise that the key will be below its cap after that single reset.

A refusal is local and pre-dispatch. It remains account-neutral even when the direct WebSocket already has an open subscription socket, so denied work cannot feed its own future demand estimate. Existing reservation cleanup remains authoritative for any provisional fixed token or dollar reservation. Demand attribution remains one folded/raw statement over a single fold watermark. A compact per-account cutoff table joins both branches, so large unscoped pools do not create one SQL predicate branch per account; folded demand begins at the first complete quarter-hour inside each quota window, while retained rows in the leading partial slot and the post-watermark tail are read exactly from raw logs. Attribution uses the persisted model field as a positive generation invariant: ordinary nonempty generation models count, while the empty control/thread-goal/realtime model, file-control sentinel models, and raw model-source rows do not. This avoids a growing list of request-kind prefixes; internal warm-up still carries a generation model and remains only in the unattributed denominator. Missing evidence fails open and wakes at most one immediate per-account refresh; the staggered fleet scheduler supplies the remaining evidence without request-triggered fan-out.
