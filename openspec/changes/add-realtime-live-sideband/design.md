...
Successful call creation already used a valid credential. Account selection may reuse a bounded routing-input cache, while forced refresh commits a detached current account snapshot, so an immediate sideband attach cannot safely decrypt the selected ORM snapshot. After exact-owner policy selection and stream leasing, the attach therefore fresh-loads that same owner from persistent storage, rejects a now-missing or unavailable owner, and uses the fresh access token, ChatGPT identity, installation identity, and route fields. It does not call the refresh manager. A definitive HTTP handshake response is not replayed through route fallbacks; its status is returned without exposing endpoint details. A handshake `401`, `403`, transport error, or timeout is returned to the caller without refreshing, selecting another account, or changing global account health. Transport-native ping/pong liveness closes dead silent sockets without imposing an application-frame idle timeout. This preserves the account-bound contract and prevents an experimental capability failure from disrupting ordinary production traffic.
