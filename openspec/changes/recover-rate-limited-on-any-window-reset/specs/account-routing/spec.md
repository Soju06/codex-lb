...
- **AND** no other window carrying quota for the account's plan is currently exhausted
- **THEN** replica B may compare-and-set the account to `ACTIVE` before the old persisted deadline
- **AND** a successful transition clears `reset_at` and `blocked_at`
...
- **AND** no other unelapsed window carrying quota for the plan is exhausted
