...
- Preserve the existing empty-ring readiness exemption and independent liveness. Any stricter single-replica readiness policy is deferred to a separate owner-approved change.
...
- No database migration, new setting, proxy request-wire change, or dashboard surface change. Health responses add `bridge_ring.heartbeat_age_seconds` and structured failure diagnostics.
...
- `bridge-ring-membership`: Ring renewal becomes independently supervised, bounded, and readiness-visible for the local replica, with membership enforcement unchanged for nonempty rings.
