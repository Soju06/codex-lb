...
async def test_lookup_request_targets_preserves_retained_response_alias_target(
    ...
    """Durable target lookup retains the replacement response referenced by a replay alias."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
...
async def test_operation_ledger_is_fenced_and_idempotent(
    ...
    """Operation ledger mutations reject stale ownership and tolerate valid idempotent repeats."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
...
async def test_chunk_writer_rejects_stale_recovery_generation(
    ...
    """Chunk writes from superseded recovery generations cannot append to the active operation."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
...
async def test_chunk_spool_is_cleared_when_rebinding_transcript_recovery_operation(
    ...
    """Rebinding a transcript recovery operation clears the old generation's chunk spool."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
...
async def test_unknown_operation_latest_parent_fallback_requires_unique_model_match(
    ...
    """Latest-parent lookup returns a candidate only when the scoped model match is unique."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
        claim = await _claim(
...
async def test_unknown_operation_root_fallback_matches_parentless_operation(
    ...
    """Root fallback selects a parentless operation rather than a continuation from another turn."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
        claim = await _claim(
...
async def test_unknown_operation_recent_lookup_is_bounded_to_session_model_and_age(
    ...
    """Recent-operation candidates remain restricted by session, model, scope, and age."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
        claim = await _claim(
...
async def test_recent_unknown_operations_preserves_overflow_sentinel_at_limit(
    ...
    """Bounded lookup retains an overflow sentinel so callers can detect ambiguous excess candidates."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
        claim = await _claim(
...
async def test_rebind_rollback_requires_the_claimed_recovery_generation(
    ...
    """Rollback cannot undo another claimant's rebind even when the operation identity matches."""
    session = async_session_factory()
    try:
        repository = DurableBridgeRepository(session)
        ...
        first_rebind = await repository.rebind_operation_for_complete_transcript(
            ...
            rebind_claim_id="winning-rebind-claim",
        )
        assert first_rebind is not None and first_rebind.recovery_dispatch_count == 1
        assert first_rebind.rebind_claim_id == "winning-rebind-claim"

        # A losing caller can have the same expected generation but cannot
        # compensate the winner with its own unique attempt identity.
        for losing_claim in (None, "losing-rebind-claim"):
            assert not await repository.rollback_operation_before_dispatch(
                operation_id=operation_id,
                session_id=claim.id,
                instance_id="inst-rebind-cas",
                ...
                restore_rebound=True,
                expected_recovery_dispatch_count=0,
                rebind_claim_id=losing_claim,
            )
        winner = await repository.get_operation(operation_id=operation_id)
        assert winner is not None and winner.state == "submitted"
        assert winner.recovery_dispatch_count == 1
        assert winner.rebind_claim_id == "winning-rebind-claim"
        ...
        assert refunded.rebind_claim_id is None
