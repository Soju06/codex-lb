...
#### Scenario: Exhausted keyed burst settles before the terminal penalty

- **GIVEN** an owner-bound request holds an API-key usage reservation and exhausts its burst retries
- **WHEN** the original 429 is surfaced
- **THEN** its transient penalty is applied only after settlement or release is confirmed
- **AND** failed settlement and failed release leave the penalty unapplied

#### Scenario: Cancelled burst wait releases its reservation

- **GIVEN** an owner-bound keyed request is waiting before a burst retry
- **WHEN** the caller cancels the request
- **THEN** the wait is cancelled, its reservation cleanup is owned to completion, and no replacement dispatch occurs
- **AND** the interrupted burst retry does not record a transient owner penalty
