...
- Treat any destination hash as an explicit scroll intent. In particular, `/settings?advanced=1#firewall` and the `/firewall` compatibility redirect continue through the existing Settings target-scrolling path.
